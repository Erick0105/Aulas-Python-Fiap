...
def consultar():
    ...
    nome = input("Por favor digite o nome a ser pesquisado\n==>")
    lista = ler_db(nome)
    for contato_pessoa in lista:
        print(f'Nome: {contato_pessoa[0]} \n\t Telefone: {contato_pessoa[1]} \n\t Email: {contato_pessoa[2]} \n\t Nascimento: {contato_pessoa[3].strftime("%d-%B-%Y")}')

def remover_db( nome ):
    conexao = gerar_conexao_db()
    cursor = conexao.cursor()
    sql = """DELETE * FROM agenda WHERE nome LIKE :1"""
    try:
        cursor.execute(sql, (nome, ))
        conexao.commit()
        conexao.close()
...
def remover():
    print("REMOVER CONTATO DO BANCO")
    nome = input("Por favor digite o nome completo do contato a ser removido\n==>")
